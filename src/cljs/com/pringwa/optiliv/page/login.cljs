    ...
    [:<>
     ...
     [:section.hero.is-info.is-fullheight.has-background-image
      [:ul.nav
       ...
       [:li {:onclick "showForgotPassword()"} "Forgot password"]
       [:li {:onclick "showSubscribe()"} "Subscribe"]
       [:li {:onclick "showContactUs()"} "Contact us"]]
      [:div.wrapper
       [:div.rec-prism
        [:div.face.face-top
         [:div.content
         ...
         [:div.content
          ...
          [:form {:onsubmit "event.preventDefault()"}
           ...
           [:div.field
            ...
            [:span.psw {:onclick "showForgotPassword()" :style "margin-top: 5px; margin-bottom: 0px; text-align: left;"} "Forgot Password?"]]
           [:div.divider {:style "display: flex; justify-content: center; align-items: center;"} "OR"]
           [:div.social-buttons
            [:button.button.is-fullwidth {:style "background-color: #fff; border-color: #dbdbdb; display: flex;"}
             [:span.icon
              [:i.mdi.mdi-google]]
           ...
           [:span.signup {:onclick "showSignup()" :style "margin-top:5px"} "Not a user?  Sign up"]]]]
        [:div.face.face-back
         [:div.content
         ...
         [:div.content
          ...
          [:form {:onsubmit "event.preventDefault()"}
           ...
           [:div.field-wrapper
            ...
            [:label "e-mail"]]
           [:div.field-wrapper
            [:input {:type "password" :name "password" :placeholder "password" :autocomplete "new-password" :pattern "(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" :required "true" :title "Password must include at least 1 uppercase, 1 lowercase, 1 number, 1 special character,
           ...
           [:div.field-wrapper {:style "margin-top: 5px; margin-bottom: -15px"}
            [:input {:type "submit"}]]
           [:span.singin {:onclick "showLogin()"} "Already a user?  Sign in"]]]]
        [:div.face.face-left
         [:div.content
          [:h2 "Contact us"]
          [:form {:onsubmit "event.preventDefault()"}
